...
def test_check_interaction_precondition_is_satisfiable():
    """CheckInteraction has no requires clause at all as of REQ-DDI-5
    (2026-07-12) -- the precondition that used to exclude the two agents'
    apixaban cells (Gate 1c Finding 2) was removed once TreatmentIndication
    closed those cells to a provable outcome for every constructible
    indication value. check_precondition_satisfiability's "no requires
    clauses" path is itself a real, deliberate case (not a crash or a
    trivial pass by omission) -- confirmed here, not assumed, since this
    is the first method in this repo to genuinely have zero requires
    clauses after previously having one."""
    verdict, detail = check_precondition_satisfiability(SPEC, "CheckInteraction")
    assert verdict == "sat", detail


def test_check_interaction_datatype_comparisons_still_modeled_via_ensures():
    """Historical regression, still relevant after REQ-DDI-5: the original
    gap this file's module docstring describes was CheckInteraction's
    requires clause directly comparing doac/agent (simple-enum datatypes)
    against named constructors -- Gate C3's EnumSort extension was built
    to model that. The requires clause is gone now, but the same
    EnumSort-modeled comparisons live on in the ensures clauses
    (including the new treatmentIndication == AFStrokePrevention ||
    treatmentIndication == RecurrentVTEPrevention disjunctions) --
    scan_weak_postconditions (below) exercises those directly; this test
    confirms the satisfiability check itself doesn't regress to refusing
    just because the requires clause it used to model is gone."""
