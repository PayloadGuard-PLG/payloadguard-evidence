...
(classification-flag provenance) is structurally different: a
deliberate, permanent trust-boundary/process decision that nobody ever
intends to become a Dafny proof (per `PHASE1_PLAN.md`'s own framing,
reclassified as a Phase 3 *integration* concern, not a proof question)
— given `intended_method: DECLARED` instead, so its note correctly
reads "intended DECLARED, realized GAP" rather than falsely implying a
future proof is coming.
...
9 new tests: `tests/test_renal_adjustment_matrix.py` (9 tests — CLI
