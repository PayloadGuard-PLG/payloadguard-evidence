...
**Gate C5 (mutation testing) is also now built, and extended twice the
same day — zero survivors, zero unclassifiable remain across all five
operator classes.** `evidence/dafny_mutate.py` generates
ROR/LOR/AOR/LVR/COI mutants against `dosage.dfy`'s requires/ensures
clauses and `ExpectedDose`'s function body;
...
concrete follow-ups on "build both": **chain-direction-aware ROR**
(eliminates 4 formerly-unclassifiable mutants by construction, ahead of
what MutDafny itself does) and **function-body AOR** (MutDafny's own
group rule means a mutation can never introduce `/` where the original
had none — eliminating the division-by-zero false-kill risk by
construction). Then, scoped ("scope out Gate C5's LVR extension") and
built ("go") the same day: **LVR** (Literal Value Replacement) — every
numeric literal in the spec is exactly `0.0` (7 sites, audited
empirically), mutated to `± 0.01`, the clinical-precision floor from the
research, finally applied. The real run matched the scoping session's
hand-derived prediction exactly, site by site: 14 mutants, 4 filtered,
10 real-verified, all 10 genuinely killed. **Final combined real run:
56 mutants — 41 killed, 6 filtered_static, 4
filtered_chain_incompatible, 1 filtered_ar_group_incompatible, 4
filtered_magnitude_implied — zero survived, zero unclassifiable** — see
