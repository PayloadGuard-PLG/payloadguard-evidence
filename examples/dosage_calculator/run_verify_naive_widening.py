# Duplicate of run_verify.py targeting the preserved naive-widening review
# artifact (see dosage_naive_widening.py's docstring). Same duplication
# rationale as run_verify_broken.py: run_verify.py stays byte-identical to
# the reviewed original.
import json, subprocess, sys, datetime, pathlib

HERE = pathlib.Path(__file__).parent
TARGET = HERE / "dosage_naive_widening.py"
...
def main():
    ...
    (HERE / "raw_crosshair_output_naive_widening.txt").write_text(
    ...
    (HERE / "run_manifest_naive_widening.json").write_text(json.dumps(manifest, indent=2))
