...
installed Dafny binary. An initial run found 2 real survivors (below),
which were fixed the same day; `mutation_report.json`/`.md` and
`run_manifest_mutation.json`, in this same directory, record the final,
post-fix outcome of every one: 29 killed, 6 filtered as statically
trivial, 0 survived, 4 unclassifiable.
...
mutation suite re-run in full confirmed zero survivors remain (the
figures above and in `mutation_report.md`/`.json` reflect that final,
post-fix state — killed=29, filtered_static=6, unclassifiable=4,
survived=0).
