# Duplicate of run_verify.py targeting overflow_probe.py — the domain-free
# model-fidelity probe (see README). Same invocation and bounds as the
# amendment runs: `crosshair check <target> --report_all`, no bounds flags.
import json, subprocess, sys, datetime, pathlib

HERE = pathlib.Path(__file__).parent
TARGET = HERE / "overflow_probe.py"
...
def main():
    ...
    (HERE / "raw_crosshair_output_overflow_probe.txt").write_text(
    ...
    (HERE / "run_manifest_overflow_probe.json").write_text(json.dumps(manifest, indent=2))
