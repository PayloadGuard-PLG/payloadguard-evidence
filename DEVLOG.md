...
## 2026-07-22 (latest+) — Main README rewritten to current-state, development narrative removed

Rewrote `README.md` to describe the system as it currently is, with the development narrative (dates, "landed on X", Finding numbers, timed-run storytelling, "research in progress" framing, per-session decisions) removed — that history stays in `DEVLOG.md`/`HANDOFF.md`/`SYSTEM_BLUEPRINT.md` where it belongs. Kept only current-state facts: what the engine is and produces, the evidence-strength labels and the committed 20/1/7 totals, the proof-content qualifier and its 14-definitional/6-property breakdown, the six gates, the `PROVEN`-honesty mechanisms (as capabilities, ratification `PENDING`), the domain-agnostic engine (aeb_kernel as the non-medical example, no timing narrative), the ISO 14971 risk-management current state (prepared severity assessments / determinations, SME role unfilled and ratification pending), repo layout, quick start, scope, further reading. `test_readme_evidence_totals.py`'s intro-sentence regex relaxed to match the reworded (still number-accurate, matrix-recomputed) totals table. 391 tests pass.

## 2026-07-22 (latest) — ALARP determination §§1–4 prepared; honest two-role attribution; "Clinical SME" role corrected

Completed the `HAZ-GIP-1.14b` ALARP determination as a **prepared package, not a signed determination** — the honest terminal state for a POC with no SME. §§1–4 authored by Steven across dialogue, transcribed with every factual anchor verified against sources first (GIP HID 1.14's designed response is `Alarm();Log()` — an active signal, confirmed line 109; the register's "materially different failure mode" quote; the `S4` "recur or go unaddressed" harm text). §1: kernel risk-control surface exhausted (`REQ-GIP-1-8-1` PROVEN), but notification is not a kernel property and the gap is *buildable elsewhere*, not unsolvable. §2: ISO/TR 24971 §C.4 *practicability* — technical practicability is **satisfied** (a path can be built), so the argument rests on stage/vehicle (demonstrate notification-verification on a domain-agnostic, lower-classification device — ISA-18.2/IEC 62682 §3.1.7/§3.1.8, the same detect→annunciate split as IEC 60601-1-8, no patient, no Class II load) and survives the guard clause because a pre-market POC with **no deployment exposes no patient**. §3: clinician oversight splits into two claims — weakly real for immediate under-delivery, **not** a control for fault-recurrence (the `S4` dimension) — so §4 must not lean on undifferentiated oversight. §4: residual stated without minimization; `S4` severity-alone caps at `ALARP` at this stage, never `Acceptable`.

**Attribution corrected to what it honestly is.** Steven's role was never to make an SME's judgment but to *make it easier for an SME* — so the sign-off is two-role: **Prepared by PayloadGuard Research (Steven/DarkVader-PLG)** / **SME determination `_PENDING_`** (awaits qualified clinical-regulatory review; informal guidance from practising clinical contacts informs the preparation but cannot ratify it during development). Status `PREPARED FOR SME REVIEW`. The doc header's "belongs to Steven as named Clinical SME" overclaim is fixed to the preparer/SME split, and `RISK_MANAGEMENT_PLAN.md` Section 2 corrected (superseded in place): the Clinical SME role is **unfilled**, not filled by a non-SME; a "Determination preparer / risk-evidence owner" row added. Stated downstream consequence: the 2026-07-15 "Steven, the named Clinical SME, scored…" severity records are, under this correction, *prepared* assessments awaiting SME confirmation — values stand as preparation, ratification does not yet exist. This is the drafter≠checker boundary (no self-recorded Gate C6 sign-off) applied to risk acceptance.

**The correction was carried through the whole repo, not just Section 2** ("be honest, don't overclaim"): every current-state assertion that Steven "is the named Clinical SME" was fixed inline (`README.md`, `SYSTEM_REFERENCE.md`, `RISK_MANAGEMENT_PLAN.md` §4.1/§4.2/§5/Path-to-sign-off, `HAZARD_REGISTER.md` intro+footer, `RISK_MANAGEMENT_FINDINGS.md`), and the current-state narrative docs that carry preserved historical blocks (`KNOWN_LIMITATIONS.md`, `SYSTEM_BLUEPRINT.md`, `HANDOFF.md`, `examples/dosage_calculator/README.md`) each got a governing role-correction note at the top rather than rewriting dated snapshots. Future-authority phrasings ("a real ALARP determination *from Steven as the named Clinical SME*") were corrected to "*from a qualified SME*" — Steven prepares it, the SME determines it. Dated change-log / append-only DEVLOG entries are left as historical record, governed by the corrections. The `polish_lint` gate caught one real downstream effect during the pre-flight — `SYSTEM_REFERENCE.md` forbids dated narrative outside its header, so its correction was reworded current-state-only. 391 tests pass; `hazard_id_lint` and `polish_lint` clean.

## 2026-07-22 (later still) — packaging fix (pymupdf extra) + ISA-18.2 source archived, from an external system analysis

Steven supplied an outside system-analysis PDF (`suggested_improvements.pdf`, 9-item action plan) and asked me to consider it carefully for hardening — verified against the live repo, not trusted. It corrected one of *my* stale assumptions (there **is** a `pyproject.toml` with a real `plg-evidence` console script and a `test_packaging.py` guarding it), and it surfaced one genuine gap it didn't fully see: `pymupdf` was pinned in `requirements.txt` (so CI works) but absent from `pyproject.toml`, so `pip install .` of the package left the citation-audit script (`verify_tr24971_iec62304_citations.py`, added in PR #79) unable to import `fitz`. **Fixed as an optional extra** — `[project.optional-dependencies] citations = ["pymupdf==1.28.0"]` — matching this repo's own convention that tooling an installed CLI doesn't need to *run* (pytest) is an extra, not a core dependency; the script already imports `fitz` lazily and errors cleanly, and the install hint now names the extra. The analysis's other claims triaged: `loguru` "missing dependency" is **false** (imported nowhere — a hallucination the verify-don't-trust rule caught); Gate C3v4 spec-stripping is real but the already-tracked deferred item; CI Type-2 conflict scan and blind-review CI enforcement are plausible future hardening, left for Steven to scope; the rest (Docker/quickstart/CSV/scalability) is cosmetic.

Also archived `sources/ISA-18.2-2016.PDF` (**ANSI/ISA-18.2-2016**, committed to `main` by Steven) into the `sources/README.md` inventory, read directly (its §3.1.7 alarm / §3.1.8 annunciation definitions verified verbatim — a domain-agnostic analog of IEC 60601-1-8's ALARM CONDITION → ALARM SIGNAL, with a secondary-source paraphrase corrected: the primary says "*timely* response"). It grounds the notification-verification architecture-generalization direction the `HAZ-GIP-1.14b` ALARP determination points to. Branch rebased onto `main` to bring the file in. The ALARP determination itself remains paused at §2 (Steven's direction pending). Full suite green; `hazard_id_lint` and `test_packaging` clean.

## 2026-07-22 (later) — Finding 5 resolved (Option C, two-track); ALARP determination §0 filled

Began the `HAZ-GIP-1.14b` ALARP determination. Its §0 (added in the prior entry's work) mandates that `RISK_MANAGEMENT_FINDINGS.md` Finding 5 be closed — or explicitly not relied on — *before* Sections 1–4 can be written, so severity-alone can't be adopted by omission. Facilitation only: I assembled the decision landscape and the verified mechanical consequences (§4.3's matrix has no `Acceptable` cell for S4; RMP Section 5 defines no path from a `GAP` evaluation to device-level `Tolerable`, so §0's path (b) was never actually available and (a) is the honest route), pulled the verbatim §5.5.3 text, and recorded **Steven's decision: Option C — two-track.** Severity-alone for the zero-evidence hazards (`HAZ-GIP-1.2b`, `HAZ-GIP-1.14b`), full P×S matrix retained for hazards with genuinely estimable probability.

Finding 5 closed in `RISK_MANAGEMENT_FINDINGS.md` (ledger row, section heading Open→Resolved with a dated closure transcribing the decision, the "Not decided"/Summary items updated). ALARP `§0` filled: path (a), Option C, with the concrete consequence stated — `HAZ-GIP-1.14b` leaves the matrix and is evaluated on its `S4` severity alone (§5.5.3), and **Sections 1–4 _are_ that severity-alone acceptability determination**, not pre-judged here. Register + RMP §4.3 carry section-level "Superseded 2026-07-22" banners (repo's superseded-not-corrected convention): the two zero-evidence hazards' cells are no longer matrix-`GAP` waiting on a probability band but "severity-alone track, per-hazard determination" — `HAZ-GIP-1.14b`'s is this ALARP doc (in progress), `HAZ-GIP-1.2b`'s is a separate pending determination (flagged, not resolved in this sitting). No acceptability verdict asserted for either — only the change of procedure. 391 tests pass; `hazard_id_lint` clean. Still `_PENDING_`: ALARP §§1–4 and sign-off (Steven's judgment), and `HAZ-GIP-1.2b`'s own severity-alone determination.

## 2026-07-22 — `HAZ-GIP-1.14b` (S4) risk sync + TR 24971 citation corrections + a runnable citation-verification manifest

Steven's severity work happened in a separate sandbox clone and was transferred here as a reviewed patch — not re-litigated. **The `HAZ-GIP-1.14b` split**: `HAZ-GIP-1.14` (reverse-delivery reflux) is narrowed to the kernel-proven delivered-dose question, and its real residual — a reverse-flow fault clamps to zero but no clinician is ever alerted — splits out as `HAZ-GIP-1.14b`, structurally parallel to `HAZ-GIP-1.2b`. Steven scored it **`S4 — Critical`** (`AskUserQuestion`, consequence-only per §4.1), this register's first divergence from `S3`, confirmed with him as a genuine consequence judgment ("critical *if* it occurs," not an inevitability claim). Probability left `GAP` on the same Finding 5 basis as `HAZ-GIP-1.2b`. Device-level residual risk unchanged (`Unacceptable` already), but two rows now block on Finding 5 instead of one, and `HAZ-GIP-1.14b` cannot reach `Acceptable` even if Finding 5 resolves — §4.3's matrix has no `Acceptable` cell for S4 at any band. The structural basis for the split was verified directly, not assumed: `REQ-GIP-1-8-1` carries no `system_scope` field in `metadata.a.yaml` (unlike `REQ-GIP-1-4-12`), so the notification gap isn't even named at requirement level. `HAZARD_REGISTER.md`/`RISK_MANAGEMENT_PLAN.md`'s "all five/every hazard: S3" claims are superseded in place (pointer, not silent edit), count corrected 5→6.

**`ALARP_DETERMINATION.md`** (new, `HAZ-GIP-1.14b`, PENDING template — structure only, Steven authors the judgment) lands with the primary-source citation corrections this session's direct read of `sources/CEN-ISO-TR-24971-2020-E.docx` forced: the standard **never uses the acronym "ALARP"** (spells out "as low as reasonably practicable") and contains **no "gross disproportion"/case-law test** — that framing was UK case law (Edwards v. National Coal Board), not TR 24971. §2 is rebuilt on the standard's actual basis — Annex **C.4** *practicability* (technical: reduce regardless of cost; economic: without making the device an unsound economic proposition, with the explicit guard that economic practicability must not justify accepting unnecessary risk). Clause map confirmed against the primary text: §C.2 policy (ALARP-naming, → ISO 14971 §4.2 NOTE 1), §C.3 criteria (severity-alone, Finding 5's anchor), §C.4 risk control (practicability). A new §0 forces the Finding 5 question (resolve it, or name the mechanism by which a `GAP` hazard reaches device-level `Tolerable`) to be answered before §§1–4, so severity-alone can't be adopted by omission.

**Citation-verification manifest, made actually runnable.** `evidence/verify_tr24971_iec62304_citations.py` (from the sandbox) re-checks all 10 TR 24971 / IEC 62304 / ISO 14971 prose claims as verbatim substrings of the committed sources via `citation_gate.py`. As delivered it couldn't run — it shelled out to LibreOffice to render the docx, which fails here ("source file could not be loaded"), crashing before any check. Reworked to read the docx directly (unzip → `word/document.xml` `<w:t>` runs); no external binary. Its stated reason for the LibreOffice detour ("Word auto-numbering needed") didn't hold — no claim quotes a clause number. All 10 now CONFIRMED end-to-end. **Source-filename typo fixed**: `sources/ISO-14691-2019-E.pdf` → `ISO-14971-2019-E.pdf` (transposed digits; the file is a valid 7-page ISO 14971:2019 preview), script path + comment updated to match. `pymupdf==1.28.0` added to `requirements.txt` (the committed script's dependency was undeclared). New `tests/test_tr24971_iec62304_citations.py` (3): all-citations-confirmed, script exits 0, and a regression guard that the corrected filename is the one present and the typo filename is gone. TEST_CATALOG regenerated (365). Full suite: **391 passed**; `hazard_id_lint` clean. Everything above was verified in a throwaway worktree off `origin/main` before landing on-branch.

## 2026-07-21 (later still) — Attestation template v2.1: related-requirement block (verified, then built)

A narrow follow-up spec from Steven (`contract_attestation_v2_1_related_requirements.md`, designed against `b484428`), verified before building per this repo's discipline. Problem: `_matched_rows` keys on `{spec}.dfy::{decl}` exactly (correctly — it's what stops `dosage.py::` implementation locations leaking in), but that drops a requirement the contract genuinely *partially* formalizes when the requirement's only recorded evidence is implementation-level. dosage's **REQ-DOSE-003** ("finite, in-range result") is the sole case across all four examples: its `ensures 0.0 <= dose <= maxSafeDoseMgPerHr` IS the in-range half (the finite half is the documented CrossHair reals-vs-IEEE gap), yet its matrix evidence is `dosage.py::calculate_hourly_dose` only, so it appeared *nowhere* in the attestation — the reviewer would answer Gap-if ("could every clause hold while the requirement is still violated?") against a target left off the page.

**Verified against the live repo (HEAD == the spec's baseline `b484428`) before a line was written:** ran the proposed `_related_rows` rule across all four examples × every frozen declaration — it yields *exactly* `{(dosage, CalculateHourlyDose, REQ-DOSE-003)}`, zero overlap with any primary `_matched_rows` result, nothing else. The other examples' absent rows are all prose-only/process-decision (renal REQ-RENAL-3/4/6/7/8, aeb REQ-AEB-9/10 — no `::` location, correctly never matched). The spec was exact as written; no corrections needed (unlike v2).

**Built as specified.** `_normalize_symbol` (underscore/case-insensitive, deliberately crude) + `_related_rows` (rows mapped *nowhere* in this spec whose implementation-location symbol normalizes to this declaration's name); a new `_RELATED_REQUIREMENT_NOTE` ("ⓘ Not proven here — ... context for your judgment, never a proof claim"); the generator emits the block after the primary requirement, before the definitional banner (a related row is never definitional-banner input by construction); the checker requires both the full rendered related line and the note in-section (same tamper-resistance as the primary/banner blocks). `_matched_rows`/`_is_definitional`/the drift-pin transition untouched. **§8 acceptance check passed exactly: regenerating all four artifacts, only `contract_attestation_dosage.md` changed — the other three byte-identical**, proving the rule is no looser than specified. 7 new tests (render, the exact-known-set regression guard, prose-only-never-renders, primary/related disjointness, both deletion tamper cases, no-block-when-empty). Full suite: 388 passed.

## 2026-07-21 (later) — Ratification template v2: defeater-based, from Steven's research spec, with four verified corrections

Steven supplied an implementation spec (`contract_attestation_v2_spec.md`, designed with a separate Claude research session against HEAD `e222e73`) replacing the v1 passive `Adopted? yes/no` with an **eliminative, production-based ratification**, grounded in the assurance-case literature (SEI eliminative argumentation's defeater taxonomy; Assurance 2.0's unresolved-doubt gating — structurally what `_PENDING_` already does) and the reading-technique evidence (Basili et al.: checklist reading no better than ad hoc; *active production* is what helps). The three defeater classes distribute to the layers that already own them: undermining = Component D's blind constant review (folded in, unchanged); rebutting = new per-declaration **Wrong-if** production field; undercutting = new per-declaration **Gap-if**. The artifact now auto-assembles everything the judgment needs — mapped requirement text from the traceability matrix, source-cited literals with verbatim quotes, and a warning banner where the recorded proof is *definitional* (directing reviewer doubt exactly where mechanical evidence is weakest) — so the machine does all lookup and the human performs only the judgment no machine can. Requirement text becomes expected content in each declaration's section, so a requirement-text change flags the artifact structurally stale without a second hash. The spec also caught a real latent trap I'd shipped: the byte-exact drift pin could only survive while artifacts were PENDING — the first signature would have broken the suite with no designed successor. v2's transition: byte-pin while unsigned; structural invariants (hash currency, frozen content, requirement text, field markers) always.

**Verified before building, per this repo's discipline — the spec's own §8 checklist plus its claims C1–C8 checked directly against the live repo. Four corrections found and applied:** (1) *Row-only requirement matching fails dosage and renal* — dosage's row `code_location` names the Python implementation (`dosage.py::calculate_hourly_dose`), and three renal functions appear at evidence level only, so the spec's row-only rule would have stamped the safety-critical `CalculateHourlyDose` (and three renal functions) "none mapped." Matching is now row-OR-evidence-level, constrained to `{spec}.dfy::{name}` exactly. (2) *Per-row definitional banners would mislabel* — rows mix `proof_content` across declarations (REQ-RENAL-2 carries definitional `SelectFormula` beside property `CockcroftGaultCrClMlPerMin`), so the banner keys on the declaration's OWN evidence entry, not its row. (3) The §8 worry about `_LITERAL_RE` visibility dissolves — public `spec_literals` works on canonical frozen surfaces (verified on `GStage`); no new helper. (4) "None mapped" wording says "type declaration or helper/internal" so datatype blocks (which legitimately have no matrix rows) don't read as findings. **A fifth caught by the tamper tests while building:** the Adopted line's own prose mentions "Wrong-if and Gap-if" unbolded, so a bare-word per-section field check would pass on that prose after the actual field was gutted — field checks now match the full bold markers (`**Wrong-if (rebutting)?**` etc.), the stable strings §4.2 itself named.
